...
# ============================================================
# RAZA SHAH SIGNAL — V3 MULTI-STRATEGY
...
# ----------------------------
# RSI + VOLUME REVERSAL ENGINE
# Direction is selected here:
#   Oversold + bullish RSI turn + volume -> BUY
#   Overbought + bearish RSI turn + volume -> SELL
# 4H / 1H remain context only; they no longer choose trade direction.
# ----------------------------
...
# Completed 15m reversal candle must carry meaningful participation.
REVERSAL_MIN_VOL_RATIO = float(os.getenv("REVERSAL_MIN_VOL_RATIO", "1.10"))
REVERSAL_STRONG_VOL_RATIO = float(os.getenv("REVERSAL_STRONG_VOL_RATIO", "1.60"))

# Requiring RSI to fully cross back through 30/70 is safer but slower.
# Default false = extreme + clear turn is enough, while liquidity/5m/live flow
# still have to confirm before a signal becomes TRADE READY.
RSI_RECLAIM_REQUIRED = (
    os.getenv("RSI_RECLAIM_REQUIRED", "false").strip().lower()
    in ("1", "true", "yes", "on")
)
...
# V3 multi-strategy engine
# The bot can qualify trend continuation, breakout, or reversal setups.
...
state = {
    ...
    "status": "Starting V3 multi-strategy...",
    "last_scan": None,
    "next_scan": None,
    "alerts_last_scan": 0,
    "latest_signal": None,
...
def live_confirmation(side, delta, book, spread, oi):
    """
    V3 live confirmation. Spread and OI remain safety gates, while flow/book
...
# ============================================================
# RSI + VOLUME REVERSAL DIRECTION
# ============================================================

def rsi_volume_reversal_15m(symbol):
    """
    Select direction from completed 15m RSI + volume.

    BUY:
      - recent completed RSI was oversold
      - RSI turns upward by RSI_TURN_MIN
      - completed reversal candle has elevated volume
      - candle closes with bullish rejection/strength

    SELL:
      - recent completed RSI was overbought
      - RSI turns downward by RSI_TURN_MIN
      - completed reversal candle has elevated volume
      - candle closes with bearish rejection/strength

    This function selects DIRECTION only. The existing 15m liquidity setup,
    5m CHOCH trigger and live flow/book/OI confirmation remain mandatory.
    """
    c = candle_dicts(symbol, "15m", 120)

    if len(c) < 60:
        raise RuntimeError(f"Not enough 15M candles for reversal {symbol}")

    closes = [x["close"] for x in c]

    r_now = rsi(closes, 14)
    r_prev = rsi(closes[:-1], 14)
    r_prev2 = rsi(closes[:-2], 14)
    ...
    oversold_seen = min(r_prev2, r_prev) <= RSI_OVERSOLD
    overbought_seen = max(r_prev2, r_prev) >= RSI_OVERBOUGHT

    bullish_turn = (r_now - r_prev) >= RSI_TURN_MIN
    bearish_turn = (r_prev - r_now) >= RSI_TURN_MIN

    if RSI_RECLAIM_REQUIRED:
        bullish_turn = bullish_turn and r_now > RSI_OVERSOLD
        bearish_turn = bearish_turn and r_now < RSI_OVERBOUGHT

    volume_ok = vr >= REVERSAL_MIN_VOL_RATIO
    ...
    if (
        ...
        side = "BUY"
        strength = "NORMAL"
        if min(r_prev2, r_prev) <= RSI_STRONG_OVERSOLD:
            strength = "STRONG"
        if (
            min(r_prev2, r_prev) <= RSI_EXTREME_OVERSOLD
            and vr >= REVERSAL_STRONG_VOL_RATIO
        ):
            strength = "EXTREME"
        ...
        side = "SELL"
        strength = "NORMAL"
        if max(r_prev2, r_prev) >= RSI_STRONG_OVERBOUGHT:
            strength = "STRONG"
        if (
            max(r_prev2, r_prev) >= RSI_EXTREME_OVERBOUGHT
            and vr >= REVERSAL_STRONG_VOL_RATIO
        ):
            strength = "EXTREME"

    reason = "READY" if side else "NO_RSI_VOLUME_REVERSAL"
...
# ============================================================
# V3 MULTI-STRATEGY 15M SETUP SELECTOR
# ============================================================

def multi_strategy_setup_15m(symbol, reg, st):
    """Choose the strongest currently valid 15m setup.

    Priority is breakout -> trend continuation -> RSI/volume reversal.
    The returned object keeps the old setup keys so dashboard, quality score,
    and dynamic risk remain backward compatible.
    """
    c = candle_dicts(symbol, "15m", 120)
    if len(c) < 60:
        raise RuntimeError(f"Not enough 15M candles for V3 setup {symbol}")
    ...
    if (
        volatility_ok
        and vr >= BREAKOUT_MIN_VOL_RATIO
        and bullish
        and price > prior_high * (1 + BREAKOUT_BUFFER_PCT)
        and price > e20
    ):
        return {
            "valid": True, "side": "BUY", "strategy_type": "BREAKOUT",
            "setup": "15M_HIGH_BREAKOUT", "price": price,
            "prior_high": prior_high, "prior_low": prior_low,
            "liquidity_level": prior_high,
            "sweep_extreme": min(last["low"], prev["low"], e20),
            "atr": atr15, "atr_pct": atr_pct, "vol_ratio": vr,
            "volatility_ok": volatility_ok, "rsi_15m": r_now,
        }
    ...
    if (
        ...
        and vr >= BREAKOUT_MIN_VOL_RATIO
        and bearish
        and price < prior_low * (1 - BREAKOUT_BUFFER_PCT)
        and price < e20
    ):
        return {
            "valid": True, "side": "SELL", "strategy_type": "BREAKOUT",
            "setup": "15M_LOW_BREAKDOWN", "price": price,
            "prior_high": prior_high, "prior_low": prior_low,
            "liquidity_level": prior_low,
    ...
    # 2) Trend continuation / pullback resumption
    buy_trend = (
        volatility_ok and vr >= TREND_MIN_VOL_RATIO
        and reg.get("regime") == "BULL"
        and st.get("structure") in ("HH_HL", "RANGE")
        and e20 > e50 and price > e20
        and 45 <= r_now <= 68
        and bullish and last["close"] >= prev["close"]
    )
    sell_trend = (
        volatility_ok and vr >= TREND_MIN_VOL_RATIO
        and reg.get("regime") == "BEAR"
        and st.get("structure") in ("LH_LL", "RANGE")
        and e20 < e50 and price < e20
        and 32 <= r_now <= 55
        and bearish and last["close"] <= prev["close"]
    )

    if buy_trend:
        return {
            "valid": True, "side": "BUY", "strategy_type": "TREND",
            "setup": "15M_TREND_CONTINUATION", "price": price,
            "prior_high": prior_high, "prior_low": prior_low,
            "liquidity_level": e20,
            "sweep_extreme": min(last["low"], prev["low"], e20),
            "atr": atr15, "atr_pct": atr_pct, "vol_ratio": vr,
    ...
    if sell_trend:
        return {
            "valid": True, "side": "SELL", "strategy_type": "TREND",
            "setup": "15M_TREND_CONTINUATION", "price": price,
            "prior_high": prior_high, "prior_low": prior_low,
            "liquidity_level": e20,
            "sweep_extreme": max(last["high"], prev["high"], e20),
    ...
    # 3) Reversal / liquidity setup (old V2 edge retained as one regime)
    reversal = rsi_volume_reversal_15m(symbol)
    if reversal.get("valid"):
        side = reversal["side"]
        liq = liquidity_setup_15m(symbol, side)
        if liq.get("valid"):
            liq["side"] = side
            liq["strategy_type"] = "REVERSAL"
            liq["reversal"] = reversal
            liq["rsi_15m"] = reversal.get("rsi_now")
            return liq

    return {
        "valid": False, "side": None, "strategy_type": "NONE",
        "setup": "NONE", "price": price,
        "prior_high": prior_high, "prior_low": prior_low,
        "liquidity_level": price, "sweep_extreme": price,
        "atr": atr15, "atr_pct": atr_pct, "vol_ratio": vr,
        "volatility_ok": volatility_ok, "rsi_15m": r_now,
        "reversal": reversal,
    }
...
def _deep_scan_one(item):
    ...
    try:
        ...
        side = setup["side"]
        strategy_type = setup.get("strategy_type", "V3")

        # Optional BTC context filter. Default remains enabled, but SIDEWAYS
        # BTC permits both directions.
        ...
        if not trigger["valid"]:
            ...
            return {
                "symbol": symbol, "side": side, "blocked": "5M_TRIGGER_WAIT",
        ...
        risk = dynamic_risk(price, side, setup)
        ...
        reversal = setup.get("reversal") or {}
        candidate = {
            ...
            "rsi_prev_15m": round(float(reversal.get("rsi_prev") or 0), 2),
            "rsi_turn_15m": round(float(reversal.get("rsi_turn") or 0), 2),
            "reversal_strength": reversal.get("strength", strategy_type),
...
def scan_once():
    ...
    scan_log("================================")
    scan_log("RAZA V3 MULTI-STRATEGY BITGET FUTURES SCAN START")

    with state_lock:
        state["status"] = (
            f"V3 24/7 scanning Top {TOP_COINS} Bitget Futures..."
        )
    ...
    if not in_ksa_session():
        ...
        with state_lock:
            ...
            state["status"] = (
                f"V3 waiting for optional KSA session "
        ...
        for future in as_completed(futures):
            ...
            if (
                ...
                telegram_async(
                    ...
                    f"V3 Closed: {p['total_trades']}\n"
                    f"V3 Win Rate: {p['win_rate']:.2f}%\n"
                    f"🔗 {APP_URL}"
                )
    ...
    with state_lock:
        ...
        if alerts:
            state["status"] = (
                f"V3: {alerts} auto paper trade(s) added"
            )
            ...
            state["status"] = (
                f"V3 scanning — main block: {top_block}"
            )
...
def scanner_loop():
    ...
    telegram_async(
        ...
        f"Logic: 15M RSI Extreme + Volume Turn → 15M Liquidity → "
...
def api_status():
    ...
    x["dashboard"] = {
        "strategy_version": STRATEGY_VERSION,
        "logic": (
            "15m RSI overbought/oversold + volume turn → 15m liquidity "
