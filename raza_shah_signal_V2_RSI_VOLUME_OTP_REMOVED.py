...
REVERSAL_MIN_VOL_RATIO = float(os.getenv("REVERSAL_MIN_VOL_RATIO", "1.25"))
...
def _deep_scan_one(item):
    ...
    try:
        # ----------------------------------------------------
        # DIRECTION: RSI EXTREME + TURN + REVERSAL VOLUME
        # ----------------------------------------------------
        ...
        # 4H + 1H are retained for dashboard/context only.
        # They no longer choose direction or hard-block a valid reversal.
        reg = regime_4h(symbol)
        st = structure_1h(symbol)

        # ----------------------------------------------------
        # EXISTING 15M LIQUIDITY CONFIRMATION
        # ----------------------------------------------------
        ...
        # Reversal participation is intentionally stricter than the old
        # generic MIN_VOL_RATIO gate.
        if float(setup.get("vol_ratio") or 0) < REVERSAL_MIN_VOL_RATIO:
            return {
                "symbol": symbol,
                "side": side,
                "blocked": "REVERSAL_VOLUME",
                "score": 0,
                "lm": lm,
                "reversal": reversal,
                "regime": reg,
        ...
        # ----------------------------------------------------
        # LIVE ORDER BOOK / EXECUTED FLOW / OI
        # ----------------------------------------------------
        ...
        candidate = {
            ...
            # Dashboard compatibility: original fields remain present.
            "regime_4h": reg["regime"],
            "structure_1h": st["structure"],
            "setup_15m": setup["setup"],
            "trigger_5m": trigger["trigger"],

            # New reversal diagnostics.
            "rsi_15m": round(float(reversal["rsi_now"]), 2),
            "rsi_prev_15m": round(float(reversal["rsi_prev"]), 2),
            "rsi_turn_15m": round(float(reversal["rsi_turn"]), 2),
