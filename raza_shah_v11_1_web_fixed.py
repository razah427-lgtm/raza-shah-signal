...
# ============================================================
# WEB DASHBOARD — NO TELEGRAM OTP / NO LOGIN SECURITY
# ============================================================
...
<title>Raza Shah Signal V11</title>
<style>
    * { box-sizing: border-box; }
    ...
    .muted { color: #95a0b2; font-size: 13px; }
    .grid {
        display: grid;
    ...
    .label { color: #8f9bad; font-size: 12px; margin-bottom: 7px; }
...
<div class="wrap">
    <div class="head">
        <div>
            <h1>RAZA SHAH SIGNAL — V11</h1>
            <div class="muted">RSI + Volume Auto Execution · Bitget USDT-M Futures</div>
        </div>
        <div class="pill" id="clock">Loading...</div>
    </div>
    ...
    <div class="grid">
        ...
        <div class="card">
            <div class="label">Auto Trading</div>
            <div class="value" id="autoTrading">...</div>
        </div>
        <div class="card">
            <div class="label">Dry Run</div>
            <div class="value" id="dryRun">...</div>
        </div>
        <div class="card">
        ...
        <div class="card">
            ...
            <div class="value" style="font-size:14px" id="lastScan">—</div>
        </div>
        <div class="card">
            <div class="label">Daily Realized Loss</div>
            <div class="value" id="dailyLoss">$0.00</div>
        </div>
    </div>

    <div class="section card">
        <div class="label">Latest Signal</div>
        <pre id="latestSignal">No signal yet.</pre>
    </div>

    <div class="section card">
        <div class="label">Scanner Error</div>
        <pre id="lastError">None</pre>
    </div>

    <div class="section card">
        <div class="label">Top Watchlist</div>
        <pre id="watchlist">Waiting for first scan...</pre>
    </div>
</div>

<script>
function yesNo(v) {
    return v ? '<span class="green">ON</span>' : '<span class="red">OFF</span>';
}
async function refresh() {
    try {
        ...
        document.getElementById('running').innerHTML =
            s.running ? '<span class="green">RUNNING</span>' : '<span class="red">STOPPED</span>';

        document.getElementById('autoTrading').innerHTML = yesNo(s.auto_trading);
        document.getElementById('dryRun').innerHTML =
            s.dry_run ? '<span class="yellow">TEST MODE</span>' : '<span class="green">LIVE</span>';

        document.getElementById('watchlistCount').textContent =
            Array.isArray(s.watchlist) ? s.watchlist.length : 0;

        document.getElementById('lastScan').textContent = s.last_scan || 'Waiting...';
        document.getElementById('dailyLoss').textContent =
            '$' + Number(s.daily_realized_loss || 0).toFixed(2);

        document.getElementById('latestSignal').textContent =
            s.latest_signal ? JSON.stringify(s.latest_signal, null, 2) : 'No signal yet.';

        document.getElementById('lastError').textContent =
            s.last_error || 'None';

        document.getElementById('watchlist').textContent =
            Array.isArray(s.watchlist) && s.watchlist.length
                ? s.watchlist.slice(0, 30).join(', ')
...
# IMPORTANT:
# Gunicorn imports this module, so start the scanner at import time.
start_background()
