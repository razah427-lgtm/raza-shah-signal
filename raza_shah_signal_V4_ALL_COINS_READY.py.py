...
STRATEGY_VERSION = "V7_NO_CONFUSION_TREND"
...
# No Confusion Pro Trend System settings (TradingView logic port)
TREND_SIGNAL_INTERVAL = os.getenv("TREND_SIGNAL_INTERVAL", "15m").strip() or "15m"
TREND_EMA_FAST = int(os.getenv("TREND_EMA_FAST", "50"))
TREND_EMA_SLOW = int(os.getenv("TREND_EMA_SLOW", "200"))
TREND_ST_ATR_LEN = int(os.getenv("TREND_ST_ATR_LEN", "10"))
TREND_ST_ATR_MULT = float(os.getenv("TREND_ST_ATR_MULT", "3.0"))
TREND_ADX_LEN = int(os.getenv("TREND_ADX_LEN", "14"))
TREND_ADX_SMOOTH = int(os.getenv("TREND_ADX_SMOOTH", "14"))
TREND_ADX_MIN = float(os.getenv("TREND_ADX_MIN", "20.0"))
TREND_ATR_LEN = int(os.getenv("TREND_ATR_LEN", "14"))
TREND_SL_ATR_MULT = float(os.getenv("TREND_SL_ATR_MULT", "1.2"))
TREND_TP1_RR = float(os.getenv("TREND_TP1_RR", "1.5"))
TREND_TP2_RR = float(os.getenv("TREND_TP2_RR", "2.5"))
TREND_TP3_RR = float(os.getenv("TREND_TP3_RR", "4.0"))
TREND_COOLDOWN_BARS = int(os.getenv("TREND_COOLDOWN_BARS", "8"))
    ...
    try:
        with get_db() as conn:
            with conn.cursor() as cur:
                ...
                # V7 staged target fields. Existing dashboard remains compatible with `tp` as final TP3.
                cur.execute("""ALTER TABLE trade_results ADD COLUMN IF NOT EXISTS tp1 DOUBLE PRECISION""")
                cur.execute("""ALTER TABLE trade_results ADD COLUMN IF NOT EXISTS tp2 DOUBLE PRECISION""")
                cur.execute("""ALTER TABLE trade_results ADD COLUMN IF NOT EXISTS tp3 DOUBLE PRECISION""")
                cur.execute("""ALTER TABLE trade_results ADD COLUMN IF NOT EXISTS tp1_hit BOOLEAN DEFAULT FALSE""")
                cur.execute("""ALTER TABLE trade_results ADD COLUMN IF NOT EXISTS tp2_hit BOOLEAN DEFAULT FALSE""")
                cur.execute("""ALTER TABLE trade_results ADD COLUMN IF NOT EXISTS remaining_notional DOUBLE PRECISION""")
...
def _rma_series(values, period):
    """Wilder RMA series compatible with Pine's ta.rma seed/recurrence."""
    n = len(values)
    out = [None] * n
...
def _atr_series(candles, period):
    return _rma_series(true_range_series(candles), period)


def _adx_series(candles, di_len=14, smooth=14):
    n = len(candles)
    if n == 0:
        return []
    plus_dm = [0.0] * n
    ...
    tr_rma = _rma_series(true_range_series(candles), di_len)
    plus_rma = _rma_series(plus_dm, di_len)
    minus_rma = _rma_series(minus_dm, di_len)
    dx = [0.0] * n
    valid_dx = [None] * n
    for i in range(n):
        if tr_rma[i] is None or plus_rma[i] is None or minus_rma[i] is None:
            continue
        den_tr = max(float(tr_rma[i]), 1e-12)
        plus = 100.0 * float(plus_rma[i]) / den_tr
        minus = 100.0 * float(minus_rma[i]) / den_tr
        dx[i] = 100.0 * abs(plus - minus) / max(plus + minus, 1.0)
        valid_dx[i] = dx[i]
    # Pine ta.rma(dx, smooth) starts once enough valid DI/DX values exist.
    first = next((i for i,v in enumerate(valid_dx) if v is not None), None)
    out = [None] * n
    if first is None:
        return out
    vals = [float(v) for v in valid_dx[first:] if v is not None]
    r = _rma_series(vals, smooth)
...
def _supertrend_series(candles, atr_period=10, multiplier=3.0):
    """Return (line, bullish) arrays using standard TradingView-style Supertrend."""
    n = len(candles)
    ...
    for i, c in enumerate(candles):
        ...
        if i == 0 or final_upper[i-1] is None:
            ...
            pu = final_upper[i-1]
            pl = final_lower[i-1]
            final_upper[i] = basic_upper if (basic_upper < pu or prev_close > pu) else pu
            final_lower[i] = basic_lower if (basic_lower > pl or prev_close < pl) else pl
            prev_bull = bool(bull[i-1])
            if prev_bull:
                bull[i] = False if c["close"] < final_lower[i] else True
            else:
                bull[i] = True if c["close"] > final_upper[i] else False
        line[i] = final_lower[i] if bull[i] else final_upper[i]
    return line, bull


def no_confusion_trend_signal(symbol):
    """Exact decision intent of the Pine indicator, evaluated on completed candles only."""
    need = max(TREND_EMA_SLOW + 60, 280)
    candles = candle_dicts(symbol, TREND_SIGNAL_INTERVAL, need, drop_live=True)
    if len(candles) < TREND_EMA_SLOW + 5:
        return {"valid": False, "reason": "NOT_ENOUGH_CANDLES"}

    closes = [c["close"] for c in candles]
    ef = _ema_series(closes, TREND_EMA_FAST)
    es = _ema_series(closes, TREND_EMA_SLOW)
    atrs = _atr_series(candles, TREND_ATR_LEN)
    adxs = _adx_series(candles, TREND_ADX_LEN, TREND_ADX_SMOOTH)
    st_line, st_bull = _supertrend_series(candles, TREND_ST_ATR_LEN, TREND_ST_ATR_MULT)

    def trend_at(i):
        if i < 0 or atrs[i] is None or adxs[i] is None or st_bull[i] is None:
            return False, False
        close = closes[i]
        ema_fast = ef[i]
        ema_slow = es[i]
        adx = float(adxs[i])
        buy = ema_fast > ema_slow and close > ema_slow and bool(st_bull[i]) and adx > TREND_ADX_MIN
        sell = ema_fast < ema_slow and close < ema_slow and (not bool(st_bull[i])) and adx > TREND_ADX_MIN
        return buy, sell

    buy_now, sell_now = trend_at(-1)
    buy_prev, sell_prev = trend_at(-2)
    side = "BUY" if (buy_now and not buy_prev) else "SELL" if (sell_now and not sell_prev) else None
    wait = not buy_now and not sell_now

    i = len(candles) - 1
    return {
        "valid": side is not None,
        "side": side,
        "wait": wait,
        "bar_time": datetime.fromtimestamp(candles[i]["ts"] / 1000, tz=timezone.utc).isoformat(),
        "close": closes[i],
        "ema_fast": ef[i],
        "ema_slow": es[i],
        "supertrend": st_line[i],
        "st_bull": bool(st_bull[i]),
        "adx": float(adxs[i] or 0.0),
        "atr": float(atrs[i] or 0.0),
        "buy_trend": buy_now,
        "sell_trend": sell_now,
        "reason": None if side else ("WAIT" if wait else "TREND_ALREADY_ACTIVE"),
    }


def _trend_trade_levels(entry, side, atr):
    if entry <= 0 or atr <= 0:
        return None
    risk_dist = atr * TREND_SL_ATR_MULT
    if side == "BUY":
        sl = entry - risk_dist
        tp1 = entry + risk_dist * TREND_TP1_RR
        tp2 = entry + risk_dist * TREND_TP2_RR
        tp3 = entry + risk_dist * TREND_TP3_RR
    else:
        sl = entry + risk_dist
        tp1 = entry - risk_dist * TREND_TP1_RR
        tp2 = entry - risk_dist * TREND_TP2_RR
        ...
        with get_db() as conn:
            with conn.cursor() as cur:
                fields = """
                    ...
                    tp1, tp2, tp3, tp1_hit, tp2_hit, remaining_notional
                """
                if v2_only:
                    cur.execute(f"""
                        SELECT {fields}
                ...
                for r in rows:
                    cur.execute("""
                        ...
                        DO UPDATE SET
                            ...
                            remaining_notional = EXCLUDED.remaining_notional
                    """, (
                        r.get("trade_id"),
                        r.get("time_utc"),
                        ...
                        float(r.get("tp1") or r.get("tp") or 0),
                        float(r.get("tp2") or r.get("tp") or 0),
                        float(r.get("tp3") or r.get("tp") or 0),
                        bool(r.get("tp1_hit", False)),
                        bool(r.get("tp2_hit", False)),
                        float(r.get("remaining_notional") if r.get("remaining_notional") not in (None, "") else r.get("position_notional") or 0),
                    ))
...
def add_open_trade(row):
    ...
    rows.append({
        ...
        "score": row["score"],
        "entry": entry,
        "tp": row.get("tp3") or row["tp"],  # dashboard-compatible final target
        "tp1": row.get("tp1") or row["tp"],
        "tp2": row.get("tp2") or row["tp"],
        "tp3": row.get("tp3") or row["tp"],
        "tp1_hit": False,
        "tp2_hit": False,
        "sl": sl,
        "status": "OPEN",
        ...
        "remaining_notional": position_notional,
        "realized_pnl": 0.0,
    })
...
def check_open_trades():
    """Monitor SL plus staged TP1/TP2/TP3. Dashboard contract remains unchanged (`tp` = TP3)."""
    rows = trade_rows(v2_only=True)
    changed = False
    messages = []
    ...
    for r in rows:
        ...
        try:
            ...
            entry = float(r["entry"])
            sl = float(r["sl"])
            tp1 = float(r.get("tp1") or r.get("tp") or 0)
            tp2 = float(r.get("tp2") or r.get("tp") or 0)
            tp3 = float(r.get("tp3") or r.get("tp") or 0)
            original_notional = float(r.get("position_notional") or 0)
            remaining = float(r.get("remaining_notional") if r.get("remaining_notional") not in (None, "") else original_notional)
            realized = float(r.get("realized_pnl") or 0.0)
            px = current_price(symbol)

            def target_hit(target):
                return px >= target if side == "BUY" else px <= target

            def pnl_for(notional, exit_px):
                if entry <= 0:
                    return 0.0
                ret = ((exit_px-entry)/entry) if side == "BUY" else ((entry-exit_px)/entry)
                return notional * ret

            # Each target releases one third of original size; TP3 releases the rest.
            tranche = original_notional / 3.0 if original_notional > 0 else 0.0

            if not bool(r.get("tp1_hit")) and tp1 > 0 and target_hit(tp1):
                qty_notional = min(remaining, tranche)
                realized += pnl_for(qty_notional, tp1)
                remaining = max(0.0, remaining - qty_notional)
                r["tp1_hit"] = True
                changed = True
                messages.append(("TP1", symbol, side, tp1, realized))

            if not bool(r.get("tp2_hit")) and tp2 > 0 and target_hit(tp2):
                qty_notional = min(remaining, tranche)
                realized += pnl_for(qty_notional, tp2)
                remaining = max(0.0, remaining - qty_notional)
                r["tp2_hit"] = True
                changed = True
                messages.append(("TP2", symbol, side, tp2, realized))

            if tp3 > 0 and target_hit(tp3):
                realized += pnl_for(remaining, tp3)
                remaining = 0.0
                r["status"] = "WIN" if realized >= 0 else "LOSS"
                r["exit_price"] = tp3
                r["closed_time_utc"] = datetime.now(timezone.utc).isoformat()
                r["remaining_notional"] = 0.0
                r["realized_pnl"] = realized
                changed = True
                messages.append(("TP3 / CLOSED", symbol, side, tp3, realized))
                continue

            stop_hit = (px <= sl) if side == "BUY" else (px >= sl)
            if stop_hit:
                realized += pnl_for(remaining, px)
                remaining = 0.0
                r["status"] = "WIN" if realized >= 0 else "LOSS"
                r["exit_price"] = px
                r["closed_time_utc"] = datetime.now(timezone.utc).isoformat()
                ...
                r["realized_pnl"] = realized
                changed = True
                messages.append(("SL / CLOSED", symbol, side, px, realized))
            else:
                r["remaining_notional"] = remaining
                r["realized_pnl"] = realized
    ...
    if changed:
        write_trade_rows(rows)
        p = performance()
        for label, symbol, side, level, realized in messages:
            telegram_async(
                f"{'✅' if 'TP' in label else '🛑'} V7 {label} — {symbol}\n"
                f"Exchange: BITGET FUTURES\n"
                f"Side: {side}\n"
                f"Level: {level:.8g}\n"
                f"Trade realized P/L so far: {'+' if realized >= 0 else '-'}${abs(realized):.2f}\n\n"
                f"📊 NO CONFUSION TREND PERFORMANCE\n"
                f"Trades: {p['total_trades']} | Wins: {p['wins']} | Losses: {p['losses']}\n"
                f"Win Rate: {p['win_rate']:.2f}% | Net P/L: ${p['net_pl']:.2f}\n"
                f"🔗 {APP_URL}"
            )
...
def _interval_seconds(interval):
    ...
    return mapping.get(str(interval).lower(), 900)


def has_recent_or_open_trade(symbol, side):
    """Single cooldown across BUY/SELL, matching the Pine system's lastSignalBar behavior."""
    now = datetime.now(timezone.utc)
    cooldown_seconds = (max(0, TREND_COOLDOWN_BARS) + 1) * _interval_seconds(TREND_SIGNAL_INTERVAL)

    for r in trade_rows(v2_only=True):
        if r.get("symbol") != symbol:
            continue
        # Do not stack another position in the same symbol in either direction.
        if r.get("status") == "OPEN":
            return True
        try:
...
def _deep_scan_one(item):
    """V7: Pine No Confusion trend logic is the sole trade gate."""
    _, symbol, lm, btc_regime = item
    try:
        sig = no_confusion_trend_signal(symbol)
        if not sig.get("valid"):
            return {
                "symbol": symbol,
                "blocked": sig.get("reason") or "WAIT",
                "score": 0,
                "lm": lm,
                "regime": {"regime": "TREND_FILTER"},
                "structure": {},
                "setup": sig,
                "error": None,
            }
        ...
        risk = _trend_trade_levels(entry, side, float(sig.get("atr") or 0))
        if not risk:
            return {"symbol": symbol, "side": side, "blocked": "RISK", "score": 0, "lm": lm, "setup": sig, "error": None}

        # Market microstructure is retained for dashboard/telemetry only; it no longer overrides the Pine signal.
        try:
            dm = depth_metrics(symbol)
            spread, book = dm if dm else (0.0, 0.0)
        except Exception:
            spread, book = 0.0, 0.0
        try:
            delta, buy, sell = flow_metrics(symbol)
        except Exception:
            delta, buy, sell = 0.0, 0.0, 0.0
        try:
        ...
        q = min(100, max(0, int(60 + min(30.0, max(0.0, adx - TREND_ADX_MIN)))))
        candidate = {
            "time_utc": datetime.now(timezone.utc).isoformat(),
            "signal_bar_time": sig.get("bar_time"),
            "strategy_version": STRATEGY_VERSION,
            "exchange": "BITGET", "data_source": "Bitget USDT-M Futures",
            "symbol": symbol, "signal": side, "score": q, "risk_label": risk_label(q),
            "price": entry, "strategy_type": "NO_CONFUSION_PRO_TREND",
            "regime_4h": "N/A", "structure_1h": "N/A", "bias_1h": side,
            "setup_15m": f"EMA{TREND_EMA_FAST}/{TREND_EMA_SLOW}+SUPERTREND+ADX",
            "location_15m": "CLOSED BAR CONFIRMED", "liquidity_event_15m": "N/A", "liquidity_tier": "N/A",
            "acceptance_15m": True, "rejection_15m": False, "reaction_15m": "TREND CONFIRMED",
            "trigger_5m": "NOT REQUIRED",
            "rsi_15m": 0.0, "rsi_prev_15m": 0.0, "rsi_turn_15m": 0.0,
            "rsi_oversold": False, "rsi_overbought": False,
            "rsi_extreme_oversold": False, "rsi_extreme_overbought": False,
            "reversal_strength": "N/A",
            "flow_delta": delta, "buy_usd_60s": buy, "sell_usd_60s": sell,
            "spread_bps": spread, "book_imb": book, "oi_change_pct": oi,
            "atr_15m_pct": (float(sig.get("atr") or 0) / entry if entry > 0 else 0.0),
            "vol_ratio": lm.get("vol_ratio_15m", 0) if isinstance(lm, dict) else 0,
            "risk_pct": risk["risk_pct"], "rr": risk["rr"],
            "tp": risk["tp"], "tp1": risk["tp1"], "tp2": risk["tp2"], "tp3": risk["tp3"], "sl": risk["sl"],
            "structural_level": sig.get("supertrend"), "hard_confirm": True,
            "blocked_reasons": [], "status": "TRADE READY",
            "confluence": 4,
            "formula_factors": [
                f"EMA{TREND_EMA_FAST} {'>' if side == 'BUY' else '<'} EMA{TREND_EMA_SLOW}",
                f"Price {'above' if side == 'BUY' else 'below'} EMA{TREND_EMA_SLOW}",
                f"Supertrend {'GREEN' if side == 'BUY' else 'RED'}",
                f"ADX {adx:.2f} > {TREND_ADX_MIN:.2f}",
                "Completed-candle transition",
            ],
            "whale_level": None, "order_block_zone": None, "target_liquidity": risk["tp3"],
            "ema_fast": sig.get("ema_fast"), "ema_slow": sig.get("ema_slow"),
            "supertrend": sig.get("supertrend"), "adx": adx,
        }
        return {"symbol": symbol, "side": side, "score": q, "candidate": candidate,
                "blocked": None, "lm": lm, "regime": {"regime": "TREND"}, "structure": {},
                "setup": sig, "trigger": {"valid": True, "trigger": "CLOSED_BAR_TREND_TRANSITION"},
                "delta": delta, "buy": buy, "sell": sell, "spread": spread, "book": book, "oi": oi, "error": None}
    except Exception as e:
...
def api_status():
    ...
    x["dashboard"] = {
        "strategy_version": STRATEGY_VERSION,
        "logic": (
            f"{TREND_SIGNAL_INTERVAL} closed candle → EMA{TREND_EMA_FAST}/EMA{TREND_EMA_SLOW} trend → "
            f"Supertrend({TREND_ST_ATR_LEN},{TREND_ST_ATR_MULT}) → ADX>{TREND_ADX_MIN:g} → ATR SL + TP1/TP2/TP3"
        ),
        "risk_rules": {
            ...
            "max_open_trades": MAX_OPEN_TRADES,
            "trade_ready": (
                "EMA trend + price vs slow EMA + Supertrend + ADX must all confirm on a closed candle. "
                "Score remains display/ranking only."
            ),
            "rr_target": RR_TARGET,
