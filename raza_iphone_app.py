...
session.headers.update({
    "User-Agent": "RAZA-SHAH-SIGNAL-BINANCE-FUTURES/4.0",
...
def binance_get(path, params=None, timeout=None, retries=3):
    if timeout is None:
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            if r.status_code in (418, 429):
                ...
                scan_log(
                    f"BINANCE RATE LIMIT HTTP {r.status_code} "
                    f"— retry in {wait}s"
                )
                time.sleep(wait)
                continue
...
def perpetual_usdt_symbols():
    now = time.time()

    if (
        _exchange_info_cache["symbols"]
        and now - _exchange_info_cache["time"] < 3600
    ):
    ...
    if not symbols:
        raise RuntimeError("No Binance USDT perpetual futures symbols found")

    _exchange_info_cache["time"] = now
...
def top_symbols():
    scan_log("TOP SYMBOLS: loading Binance USD-M Futures tickers...")

    allowed = perpetual_usdt_symbols()
    ...
    for x in tickers:
        try:
            ...
            quote_volume = float(x.get("quoteVolume") or 0)
    ...
    if not symbols:
        raise RuntimeError("No Binance top futures symbols loaded")
...
def current_price(symbol):
    ...
    if px <= 0:
        raise RuntimeError(f"No Binance ticker price for {symbol}")
...
def light_metrics(symbol):
    ...
    vols = [float(x[7]) for x in k]  # quote asset volume
...
def flow_metrics(symbol):
    ...
    for t in trades:
        try:
            ...
            # Binance aggTrade:
            # m=True => buyer is maker => aggressive taker is SELL.
...
def oi_change_pct(symbol):
    ...
    old = float(rows[-2].get("sumOpenInterestValue") or 0)
    new = float(rows[-1].get("sumOpenInterestValue") or 0)
...
def scan_once():
    ...
    scan_log("================================")
    scan_log("BINANCE FUTURES SCAN START")
    ...
    scan_log("Loading Top Binance USDT Perpetual symbols...")
    ...
    try:
        ...
        with state_lock:
            ...
            state["status"] = (
                "Binance symbol load error — retrying"
            )
            state["last_scan"] = (
                datetime.now(timezone.utc).isoformat()
            )
