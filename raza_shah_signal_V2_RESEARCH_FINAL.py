...
MIN_VOL_RATIO = float(os.getenv("MIN_VOL_RATIO", "1.05"))
...
RETEST_DISTANCE_PCT = float(os.getenv("RETEST_DISTANCE_PCT", "0.0050"))
...
# 24/7 scanning is locked for this build. Session hours remain as display metadata only.
...
def structure_1h(symbol):
    ...
    # Research-stable structure logic:
    # keep market structure important, but avoid requiring a textbook HH/HL or LH/LL
    # on every rolling block. Crypto trends often pull back before continuation.
    older = c[-36:-24]
    middle = c[-24:-12]
    recent = c[-12:]
    ...
    # Trend alignment gets the largest weight.
    if e20 >= e50:
        bull_points += 2
    if price >= e20:
        bull_points += 1
    if ema_slope >= -0.0005:
        bull_points += 1
    if mid_high >= old_high * 0.997:
        bull_points += 1
    if mid_low >= old_low * 0.995:
        bull_points += 1
    if recent_low >= mid_low * 0.992:
        bull_points += 1
    ...
    if ema_slope <= 0.0005:
        bear_points += 1
    if mid_low <= old_low * 1.003:
        bear_points += 1
    if mid_high <= old_high * 1.005:
        bear_points += 1
    if recent_high <= mid_high * 1.008:
        bear_points += 1
...
def liquidity_setup_15m(symbol, side):
    ...
    close_location = (signal_candle["close"] - signal_candle["low"]) / candle_range
    ...
    # Balanced confirmation: accept either a directional body or a clear rejection wick.
    # The liquidity level still has to be swept/retested and reclaimed/rejected.
    bullish_candle = (
        (signal_candle["close"] > signal_candle["open"] and close_location >= 0.45)
        or (lower_wick >= 0.35 * candle_range and close_location >= 0.55)
    )

    bearish_candle = (
        (signal_candle["close"] < signal_candle["open"] and close_location <= 0.55)
        or (upper_wick >= 0.35 * candle_range and close_location <= 0.45)
    )
...
def quality_score(
    ...
    # 1H structure: textbook structure receives full points;
    # balanced trend continuation receives partial points and must earn the rest
    # from liquidity, trigger and live microstructure.
    if (
        ...
        or
        (side == "SELL" and structure.get("structure") == "BEAR_TREND")
    ):
        strength = float(structure.get("strength") or 0)
        score += 15 if strength >= 85 else 12
...
def forming_performance():
    ...
    return {
        ...
        "total_loss": 0.0,
        "net_pl": 0.0,
        "note": "V2R2 measures performance only after a fully validated trade entry.",
    }
...
def score_performance():
    ...
    return {
        ...
        "note": (
            "V2R2 score is a quality/ranking metric only. "
...
def capital_summary():
    ...
    return {
        ...
        "source": "V2R2 VALIDATED TRADES ONLY",
...
def _deep_scan_one(item):
    ...
    try:
        ...
        structure_ok = (
            (
                side == "BUY"
                and st["structure"] in ("HH_HL", "BULL_TREND")
            )
            or
            (
                side == "SELL"
                and st["structure"] in ("LH_LL", "BEAR_TREND")
            )
        )
...
def api_status():
    ...
    x["dashboard"] = {
        ...
        "risk_rules": {
            ...
            "trade_ready": (
                f"All V2R2 hard filters must pass and score must be >= {MIN_TRADE_SCORE}. "
