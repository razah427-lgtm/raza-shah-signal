...
# ============================================================
# RAZA SHAH SIGNAL — V2R1 RESEARCH FINAL + DIAGNOSTICS
...
MIN_TRADE_SCORE = int(os.getenv("MIN_TRADE_SCORE", "75"))
...
def forming_performance():
    ...
    return {
        ...
        "total_loss": 0.0,
        "net_pl": 0.0,
        "note": "V2R1 measures performance only after a fully validated trade entry.",
    }
...
def score_performance():
    ...
    return {
        ...
        "note": (
            "V2R1 score is a quality/ranking metric only. "
...
# ============================================================
# V2 DIAGNOSTICS — NO CHANGE TO TRADE LOGIC
# ============================================================

_DIAG_STAGE = {
    "4H_SIDEWAYS": (10, "4H regime"),
    "BTC_CONFLICT": (15, "BTC context"),
    "1H_STRUCTURE": (30, "1H structure"),
    "15M_LIQUIDITY": (45, "15M liquidity"),
    "LOW_VOLUME": (55, "15M volume"),
    "5M_TRIGGER_WAIT": (70, "5M trigger"),
    "NO_ORDER_BOOK": (75, "order book"),
    "RISK_TOO_WIDE": (85, "risk"),
    "LIVE_CONFIRM": (90, "live flow / book"),
    "QUALITY_SCORE": (92, "quality score"),
    None: (100, "trade ready"),
}


def _diagnostic_from_result(result):
    """Build a dashboard diagnostic only. It NEVER creates a trade."""
    if not result or result.get("error"):
        return None

    blocked = result.get("blocked")
    candidate = result.get("candidate") or {}
    progress, stage = _DIAG_STAGE.get(blocked, (5, str(blocked or "unknown")))

    # If a full candidate exists, preserve its real market-quality score.
    quality = int(round(float(result.get("score") or candidate.get("score") or 0)))

    reasons = []
    for r in candidate.get("blocked_reasons") or []:
        if r and r not in reasons:
            reasons.append(str(r))
    if blocked and blocked not in reasons:
        reasons.append(str(blocked))

    side = result.get("side") or candidate.get("signal")
    lm = result.get("lm") or {}
    ...
    status = "TRADE READY" if blocked is None and candidate.get("hard_confirm") else f"WAITING — {stage}"
    ...
    return {
        ...
        # progress_score is diagnostic readiness, NOT the trade quality score.
        ...
        "vol_ratio_15m": lm.get("vol_ratio_15m"),
        "regime_4h": reg.get("regime"),
        "structure_1h": st.get("structure"),
        "setup_15m": setup.get("setup"),
        "trigger_5m": trigger.get("trigger"),
...
def scan_once():
    ...
    with ThreadPoolExecutor(
        ...
        for future in as_completed(futures):
            ...
            diag = _diagnostic_from_result(result)
            if diag:
                diagnostics.append(diag)
                if (
                    best_forming is None
                    or (diag["progress_score"], diag["quality_score"])
                    > (best_forming["progress_score"], best_forming["quality_score"])
                ):
                    best_forming = diag
    ...
    with state_lock:
        ...
        state["best_forming_setup"] = best_forming
        state["scan_diagnostics"] = sorted(
            diagnostics,
            key=lambda x: (x.get("progress_score", 0), x.get("quality_score", 0)),
            reverse=True,
        )[:10]
        state["last_scan"] = now.isoformat()
        ...
        if alerts:
            ...
            if best_forming:
                qtxt = (
                    f" | quality {best_forming['quality_score']}/100"
                    if best_forming.get("quality_score", 0) > 0
                    else ""
                )
                state["status"] = (
                    f"V2 forming: {best_forming['symbol']} "
                    f"{best_forming.get('signal') or '—'} "
                    f"| waiting {best_forming['waiting_for']}"
                    f"{qtxt} | block {best_forming.get('rejection_reason') or top_block}"
                )
            else:
                state["status"] = (
    ...
    scan_log(
        ...
        f"| forming={best_forming['symbol'] if best_forming else 'NONE'} "
