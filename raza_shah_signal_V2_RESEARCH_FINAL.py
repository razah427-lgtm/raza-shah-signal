...
BUILD_VERSION = "V10_2_2_STARTUP_ORDER_FIX_20260817"
...
# V10.2 BALANCED SMART-MONEY GATES
# 4H chooses direction. 1H neutral/pullback can be accepted; direct HTF opposition remains a hard veto.
HTF_ALLOW_NEUTRAL_1H = os.getenv("HTF_ALLOW_NEUTRAL_1H", "true").strip().lower() in ("1","true","yes","on")
HTF_MIN_1H_STRENGTH_NEUTRAL = int(os.getenv("HTF_MIN_1H_STRENGTH_NEUTRAL", "50"))
# 15m setup can be graded: textbook SMC = strong, contextual reclaim/retest = acceptable only with strong HTF + live flow.
SMC_BALANCED_ENABLED = os.getenv("SMC_BALANCED_ENABLED", "true").strip().lower() in ("1","true","yes","on")
SMC_CONTEXTUAL_RETEST_PCT = float(os.getenv("SMC_CONTEXTUAL_RETEST_PCT", "0.0100"))
SHADOW_AUDIT_ENABLED = os.getenv("SHADOW_AUDIT_ENABLED", "true").strip().lower() in ("1","true","yes","on")

# V9 SHADOW RESEARCH — diagnostics only; NEVER an entry veto.
# Goal: test liquidity-state + quarter-hour flow + cost edge before promoting anything.
V9_SHADOW_RESEARCH_ENABLED = (
    os.getenv("V9_SHADOW_RESEARCH_ENABLED", "true").strip().lower()
    in ("1", "true", "yes", "on")
)
V9_LIQ_CALM_SPREAD_BPS = float(os.getenv("V9_LIQ_CALM_SPREAD_BPS", "0.60"))
V9_LIQ_STRESSED_SPREAD_BPS = float(os.getenv("V9_LIQ_STRESSED_SPREAD_BPS", "1.50"))
V9_BOOK_STRESS_IMB = float(os.getenv("V9_BOOK_STRESS_IMB", "0.60"))
V9_QUARTER_WINDOW_SECONDS = int(os.getenv("V9_QUARTER_WINDOW_SECONDS", "45"))
V9_COST_SAFETY_MULT = float(os.getenv("V9_COST_SAFETY_MULT", "1.50"))
...
SHADOW_AUDIT_FILE = DATA_DIR / "v10_2_rejection_shadow.jsonl"
V9_SHADOW_FILE = DATA_DIR / "v9_shadow_microstructure.jsonl"
...
def liquidity_target_map_from_candles(candles, price, heat=None):
    ...
    if preferred_side=="BUY" and up:
        ...
        wall_before_target=(wall_px>price and wall_px<float(up["price"]) and wall_ratio>=LIQ_PATH_BLOCK_WALL_RATIO and wall_dist<=target_dist*LIQ_PATH_BLOCK_FRACTION)
        if wall_before_target:
            adaptive_wall={"side":"ASK","price":wall_px,"ratio":wall_ratio,"distance_pct":wall_dist}
            if wall_ratio>=LIQ_HARD_BLOCK_WALL_RATIO and wall_dist<=target_dist*LIQ_HARD_BLOCK_FRACTION:
                path_blocked=True; path_block_reason="EXTREME_ASK_WALL_NEAR_ENTRY"
        ...
        wall_before_target=(0<wall_px<price and wall_px>float(down["price"]) and wall_ratio>=LIQ_PATH_BLOCK_WALL_RATIO and wall_dist<=target_dist*LIQ_PATH_BLOCK_FRACTION)
        if wall_before_target:
            adaptive_wall={"side":"BID","price":wall_px,"ratio":wall_ratio,"distance_pct":wall_dist}
            if wall_ratio>=LIQ_HARD_BLOCK_WALL_RATIO and wall_dist<=target_dist*LIQ_HARD_BLOCK_FRACTION:
                path_blocked=True; path_block_reason="EXTREME_BID_WALL_NEAR_ENTRY"
    return {"up":up,"down":down,"up_score":round(up_score,4),"down_score":round(down_score,4),"dominance_ratio":round(ratio,4),"dominance_gap":round(gap,4),"preferred_side":preferred_side,"path_blocked":path_blocked,"path_block_reason":path_block_reason,"adaptive_wall":adaptive_wall,"atr":atr15}
...
def liquidity_hunter_risk(entry,side,setup,liq_map,heat=None,whale=None):
    ...
    # V8.1: a strong but non-extreme wall before the liquidity target becomes
    # an adaptive TP. This keeps wall protection while allowing the setup to
    # reach flow/CVD confirmation instead of being rejected immediately.
    tp_source="INFERRED_STOP_LIQUIDITY_POOL"
    aw=liq_map.get("adaptive_wall") or {}
    wall_px=float(aw.get("price") or 0)
    if wall_px>0:
        wall_buffer=max(entry*LIQ_ADAPTIVE_WALL_TP_BUFFER_PCT, atr15*0.05)
        if side=="BUY" and entry<wall_px<target_price:
            candidate_tp=wall_px-wall_buffer
            if candidate_tp>entry:
                candidate_move=(candidate_tp-entry)/entry
                candidate_rr=_net_rr_from_move(candidate_move,float(base["risk_pct"]))
                if candidate_rr>=NET_RR_MIN:
                    tp=candidate_tp; tp_source="ADAPTIVE_TP_BEFORE_ASK_WALL"
        elif side=="SELL" and target_price<wall_px<entry:
            candidate_tp=wall_px+wall_buffer
            if 0<candidate_tp<entry:
                candidate_move=(entry-candidate_tp)/entry
                candidate_rr=_net_rr_from_move(candidate_move,float(base["risk_pct"]))
                if candidate_rr>=NET_RR_MIN:
                    tp=candidate_tp; tp_source="ADAPTIVE_TP_BEFORE_BID_WALL"

    move_pct=((tp-entry)/entry) if side=="BUY" else ((entry-tp)/entry)
    net_rr=_net_rr_from_move(move_pct,float(base["risk_pct"]))
    if net_rr<NET_RR_MIN: return None
    base.update({"tp":tp,"rr":net_rr,"net_rr":net_rr,"tp_source":tp_source,"liquidity_target_price":target_price,"liquidity_target_score":float((target or {}).get("score") or 0),"liquidity_target_kind":(target or {}).get("kind"),"liquidity_target_distance_pct":float((target or {}).get("distance_pct") or 0),"adaptive_wall":aw})
    return base
...
def v9_liquidity_state(spread_bps, book_imb):
    """Shadow classifier only. It does not block or create trades."""
    spread = float(spread_bps or 0.0)
    imb = abs(float(book_imb or 0.0))
    if spread >= V9_LIQ_STRESSED_SPREAD_BPS or imb >= V9_BOOK_STRESS_IMB:
        return "STRESSED"
    if spread <= V9_LIQ_CALM_SPREAD_BPS and imb <= 0.35:
        return "CALM"
    return "NORMAL"


def v9_quarter_hour_context(now=None):
    """Describe proximity to :00/:15/:30/:45 UTC boundaries for research."""
    now = now or datetime.now(timezone.utc)
    minute_mod = now.minute % 15
    seconds_from_open = (minute_mod * 60) + now.second
    # With a 5-minute scanner, this records whether a sample landed close enough
    # to the opening boundary. A future high-frequency collector can tighten it.
    in_opening_window = seconds_from_open <= max(1, V9_QUARTER_WINDOW_SECONDS)
    return {
        "quarter_minute": now.minute - minute_mod,
        "seconds_from_quarter_open": seconds_from_open,
        "opening_window": in_opening_window,
    }


def v9_cost_edge(side, price, tp, delta, book):
    """Estimate whether the planned TP has enough gross room over trading costs.

    Diagnostic only. expected_move_proxy deliberately discounts TP distance by
    current directional flow/book quality, so it is conservative and testable.
    """
    price = float(price or 0.0); tp = float(tp or 0.0)
    if price <= 0 or tp <= 0:
        return {"gross_tp_move_pct": 0.0, "round_trip_cost_pct": _round_trip_cost_pct(),
                "expected_move_proxy_pct": 0.0, "cost_buffer_required_pct": 0.0,
                "edge_over_cost": False}
    gross = ((tp-price)/price) if side == "BUY" else ((price-tp)/price)
    gross = max(0.0, gross)
    flow_dir = max(0.0, min(1.0, _directional(delta, side)))
    book_dir = max(0.0, min(1.0, _directional(book, side)))
    confidence = max(0.25, min(1.0, 0.50 + 0.30*flow_dir + 0.20*book_dir))
    expected = gross * confidence
    cost = _round_trip_cost_pct()
    required = cost * max(1.0, V9_COST_SAFETY_MULT)
    return {
        "gross_tp_move_pct": gross,
        "round_trip_cost_pct": cost,
        "expected_move_proxy_pct": expected,
        "cost_buffer_required_pct": required,
        "edge_over_cost": expected > required,
    }


def v9_shadow_snapshot(symbol, side, price, spread, book, delta, oi, whale_delta, persistence, risk=None):
    now = datetime.now(timezone.utc)
    qctx = v9_quarter_hour_context(now)
    risk = risk or {}
    edge = v9_cost_edge(side, price, risk.get("tp"), delta, book)
    snap = {
        "time_utc": now.isoformat(),
        "symbol": symbol, "side": side,
        "liquidity_state": v9_liquidity_state(spread, book),
        "spread_bps": float(spread or 0), "book_imb": float(book or 0),
        "flow_delta": float(delta or 0), "oi_change_pct": float(oi or 0),
        "whale_delta": float(whale_delta or 0),
        "flow_persistent": bool((persistence or {}).get("persistent")),
        "avg_flow_dir": float((persistence or {}).get("avg_flow_dir") or 0),
        "avg_book_dir": float((persistence or {}).get("avg_book_dir") or 0),
        **qctx, **edge,
    }
    if V9_SHADOW_RESEARCH_ENABLED:
        try:
            with open(V9_SHADOW_FILE, "a", encoding="utf-8") as f:
                f.write(json.dumps(snap, ensure_ascii=False, default=str) + "\n")
        except Exception as e:
            scan_log(f"V9 SHADOW WRITE ERROR {symbol}: {type(e).__name__}: {e}")
        with state_lock:
            store = state.setdefault("v9_shadow_latest", {})
            store[symbol] = snap
            # Bound persisted state size.
            if len(store) > max(20, DEEP_CHECK):
                oldest = sorted(store.items(), key=lambda kv: kv[1].get("time_utc", ""))
                for k, _ in oldest[:len(store)-max(20, DEEP_CHECK)]:
                    store.pop(k, None)
    return snap
...
# Preserve blocked best-observation details for dashboard display only.
# This NEVER changes entry eligibility or TRADE READY execution logic.
# ============================================================

def _generator_observation(result):
    if not isinstance(result, dict) or result.get("error"):
        return None
    blocked = str(result.get("blocked") or "")
    side = result.get("side")
    liq = result.get("liquidity_map") or {}
    lp = result.get("liquidity_persistence") or {}
    fp = result.get("flow_persistence") or {}
    ...
    stage_score = {
        ...
        "LIQUIDITY_TARGET_NOT_PERSISTENT": 50,
        "LIQ_TARGET_AGAINST_4H_1H": 54,
        "HTF_NO_SUPPORT_FOR_LIQ_TARGET": 56,
    ...
    actual_score = int(result.get("score") or 0)
    diagnostic_score = actual_score if actual_score > 0 else stage_score
    adaptive = liq.get("adaptive_wall") or {}
    selected = (liq.get("up") if side == "BUY" else liq.get("down")) or {}

    return {
        "symbol": result.get("symbol"),
        "signal": side,
        "score": max(0, min(100, int(diagnostic_score))),
        "score_type": "QUALITY" if actual_score > 0 else "PROVISIONAL_DIAGNOSTIC",
        "provisional": actual_score <= 0,
        "hard_confirm": False,
        "status": blocked or "WAIT",
        "research_mode": "LIQUIDITY_STOP_POOL_HUNTER",
        "direction_basis": (
            "BUY_SIDE_STOPS_ABOVE_DOMINANT" if side == "BUY" else
            "SELL_SIDE_STOPS_BELOW_DOMINANT" if side == "SELL" else None
        ),
        "flow_persistence_aligned": fp.get("aligned", 0),
        "flow_persistence_required": fp.get("required", FLOW_PERSIST_REQUIRED) if fp else 0,
        "liquidity_persistence_aligned": lp.get("aligned", 0),
        "liquidity_persistence_required": lp.get("required", LIQ_TARGET_PERSIST_REQUIRED) if lp else 0,
        "liquidity_target_price": float(selected.get("price") or 0),
        "liquidity_up_score": float(liq.get("up_score") or 0),
        "liquidity_down_score": float(liq.get("down_score") or 0),
        "liquidity_dominance_ratio": float(liq.get("dominance_ratio") or 0),
        "flow_delta": float(result.get("delta") or 0),
        "book_imb": float(result.get("book") or 0),
        "oi_change_pct": float(result.get("oi") or 0),
        "wall_side": adaptive.get("side"),
        "wall_price": float(adaptive.get("price") or 0),
        "wall_ratio": float(adaptive.get("ratio") or 0),
        "wall_distance_pct": float(adaptive.get("distance_pct") or 0),
    }



def _append_shadow_audit(row):
    """Persist rejected-candidate diagnostics without affecting signal execution."""
    if not SHADOW_AUDIT_ENABLED:
        return
    try:
        payload = dict(row or {})
        payload["audit_time_utc"] = datetime.now(timezone.utc).isoformat()
        with open(SHADOW_AUDIT_FILE, "a", encoding="utf-8") as f:
            f.write(json.dumps(payload, ensure_ascii=False, default=str) + "\n")
    except Exception as e:
        scan_log(f"SHADOW AUDIT ERROR: {type(e).__name__}: {e}")
...
def scan_once():
    ...
    scan_log("================================")
    scan_log("RAZA V10.2 BALANCED SMART-MONEY + REJECTION AUDIT SCAN START")
        ...
        for future in as_completed(futures):
            ...
            if blocked:
                ...
                _append_shadow_audit({
                    ...
                    "regime_4h": result.get("regime_4h"),
                    "structure_1h": result.get("structure_1h"),
                    "setup_15m": result.get("setup_15m"),
                    "trigger_5m": result.get("trigger_5m"),
                    "flow_delta": result.get("flow_delta"),
                    "book_imb": result.get("book_imb"),
                    "oi_change_pct": result.get("oi_change_pct"),
                    "spread_bps": result.get("spread_bps"),
                })

            obs = _generator_observation(result)
            if obs and (best_observation is None or obs.get("score", 0) > best_observation.get("score", 0)):
    ...
    with state_lock:
        ...
        diag["best_observation_symbol"] = best_observation.get("symbol") if best_observation else None
        diag["best_observation_score"] = best_observation.get("score", 0) if best_observation else 0
        diag["best_observation_reason"] = best_observation.get("status") if best_observation else None
        ...
        gen_source = best or best_observation
        state["signal_generator"] = {
            "action": (
                ("LONG" if gen_source.get("signal") == "BUY" else "SHORT")
                if gen_source and gen_source.get("signal") in ("BUY", "SELL") else "WAIT"
            ),
            "symbol": gen_source.get("symbol") if gen_source else None,
            "score": gen_source.get("score", 0) if gen_source else 0,
            "score_type": gen_source.get("score_type", "QUALITY") if gen_source else "NONE",
            "provisional": bool(gen_source and gen_source.get("provisional")),
            "ready": bool(best and best.get("hard_confirm")),
            "reason": (best.get("status") if best else (best_observation.get("status") if best_observation else top_block)),
            "research_mode": (gen_source.get("research_mode") if gen_source else "LIQUIDITY_STOP_POOL_HUNTER"),
            "direction_basis": gen_source.get("direction_basis") if gen_source else None,
            "flow_persistence": (
                f"{gen_source.get('flow_persistence_aligned',0)}/{gen_source.get('flow_persistence_required',0)}"
                if gen_source else "0/0"
            ),
            "liquidity_target": gen_source.get("liquidity_target_price") if gen_source else None,
            "liquidity_up_score": gen_source.get("liquidity_up_score", 0) if gen_source else 0,
            "liquidity_down_score": gen_source.get("liquidity_down_score", 0) if gen_source else 0,
            "liquidity_dominance": gen_source.get("liquidity_dominance_ratio", 0) if gen_source else 0,
            "liquidity_persistence": (
                f"{gen_source.get('liquidity_persistence_aligned',0)}/{gen_source.get('liquidity_persistence_required',0)}"
                if gen_source else "0/0"
            ),
            "flow_delta": gen_source.get("flow_delta", 0) if gen_source else 0,
            "book_imb": gen_source.get("book_imb", 0) if gen_source else 0,
            "oi_change_pct": gen_source.get("oi_change_pct", 0) if gen_source else 0,
            "wall_side": gen_source.get("wall_side") if gen_source else None,
            "wall_price": gen_source.get("wall_price", 0) if gen_source else 0,
            "wall_ratio": gen_source.get("wall_ratio", 0) if gen_source else 0,
            "wall_distance_pct": gen_source.get("wall_distance_pct", 0) if gen_source else 0,
            "build_version": BUILD_VERSION,
        }
...
# V10.2.2: fail fast only after every background target has been defined.
_required_background_targets = {
    "scanner_loop": scanner_loop,
    "trade_monitor_loop": trade_monitor_loop,
    "hourly_status": telegram_hourly_status_loop,
    "background_watchdog_loop": background_watchdog_loop,
    "_append_shadow_audit": _append_shadow_audit,
    "ensure_background_services": ensure_background_services,
}
_missing_background_targets = [
    name for name, fn in _required_background_targets.items() if not callable(fn)
]
if _missing_background_targets:
    raise RuntimeError(
        "V10.2.2 STARTUP SELF-CHECK FAILED: "
        + ", ".join(_missing_background_targets)
    )
scan_log("V10.2.2 STARTUP SELF-CHECK PASSED")

# Start at Gunicorn/Render module boot, independent of browser requests.
