...
# ============================================================
# RAZA SHAH SIGNAL — V2R1 RESEARCH FINAL
...
BUILD_VERSION = "V2R3_BALANCED_SIGNAL_FLOW"
...
def structure_1h(symbol):
    ...
    if price >= e20: bull_points += 1
    if ema_slope >= -0.0008: bull_points += 1
    if mid_high >= old_high * 0.996: bull_points += 1
    if mid_low >= old_low * 0.994: bull_points += 1
    if recent_low >= mid_low * 0.990: bull_points += 1

    if e20 <= e50: bear_points += 2
    if price <= e20: bear_points += 1
    if ema_slope <= 0.0008: bear_points += 1
    if mid_low <= old_low * 1.004: bear_points += 1
    if mid_high <= old_high * 1.006: bear_points += 1
    if recent_high <= mid_high * 1.010: bear_points += 1

    textbook_bull = (
        mid_high >= old_high and mid_low >= old_low
        and recent_low >= mid_low * 0.997
        and price > e20 and e20 >= e50
    )
    textbook_bear = (
        mid_low <= old_low and mid_high <= old_high
        and recent_high <= mid_high * 1.003
        and price < e20 and e20 <= e50
    )

    if textbook_bull:
        structure, strength = "HH_HL", 100
    elif textbook_bear:
        structure, strength = "LH_LL", 100
    elif bull_points >= 5 and bull_points > bear_points:
        structure = "BULL_TREND"
        strength = int(round((bull_points / 7.0) * 100))
    elif bear_points >= 5 and bear_points > bull_points:
        structure = "BEAR_TREND"
        strength = int(round((bear_points / 7.0) * 100))
    else:
        structure = "RANGE"
        strength = int(round((max(bull_points, bear_points) / 7.0) * 100))

    return {
        "structure": structure, "strength": strength,
        "bull_points": bull_points, "bear_points": bear_points,
        "old_high": old_high, "old_low": old_low,
        "mid_high": mid_high, "mid_low": mid_low,
        "recent_high": recent_high, "recent_low": recent_low,
        "ema20": e20, "ema50": e50, "ema20_slope": ema_slope,
    }
...
def liquidity_setup_15m(symbol, side):
    ...
    rng = max(signal_candle["high"] - signal_candle["low"], 1e-12)
    close_loc = (signal_candle["close"] - signal_candle["low"]) / rng
    ...
    bullish_candle = (
        (signal_candle["close"] > signal_candle["open"] and close_loc >= 0.42)
        or (lower_wick >= 0.30 * rng and close_loc >= 0.52)
    )
    bearish_candle = (
        (signal_candle["close"] < signal_candle["open"] and close_loc <= 0.58)
        or (upper_wick >= 0.30 * rng and close_loc <= 0.48)
    )

    if side == "BUY":
        swept = (signal_candle["low"] <= prior_low * (1 + SWEEP_BUFFER_PCT)
                 and signal_candle["close"] > prior_low and bullish_candle)
        retest = (abs(signal_candle["low"] - prior_low) / prior_low <= RETEST_DISTANCE_PCT
                  and signal_candle["close"] > prior_low and bullish_candle
                  and signal_candle["close"] >= prev_candle["close"] * 0.998)
        valid = (swept or retest) and volatility_ok
        setup_name = "LOW_SWEEP_RECLAIM" if swept else "SUPPORT_RETEST" if retest else "NONE"
        liquidity_level = prior_low
        sweep_extreme = min(signal_candle["low"], prev_candle["low"])
    else:
        swept = (signal_candle["high"] >= prior_high * (1 - SWEEP_BUFFER_PCT)
                 and signal_candle["close"] < prior_high and bearish_candle)
        retest = (abs(signal_candle["high"] - prior_high) / prior_high <= RETEST_DISTANCE_PCT
                  and signal_candle["close"] < prior_high and bearish_candle
                  and signal_candle["close"] <= prev_candle["close"] * 1.002)
        valid = (swept or retest) and volatility_ok
        setup_name = "HIGH_SWEEP_REJECT" if swept else "RESISTANCE_RETEST" if retest else "NONE"
        liquidity_level = prior_high
        sweep_extreme = max(signal_candle["high"], prev_candle["high"])

    return {
        "valid": bool(valid), "setup": setup_name, "price": price,
        "prior_high": prior_high, "prior_low": prior_low,
        "liquidity_level": liquidity_level, "sweep_extreme": sweep_extreme,
...
def quality_score(
    ...
    if (
        ...
        or (side == "SELL" and structure.get("structure") == "LH_LL")
    ):
        score += 20
    elif (
        (side == "BUY" and structure.get("structure") == "BULL_TREND")
        or (side == "SELL" and structure.get("structure") == "BEAR_TREND")
    ):
        strength = float(structure.get("strength") or 0)
        score += 17 if strength >= 85 else 14
...
def live_confirmation(side, delta, book, spread, oi):
    ...
    # V2R1: OI is not a hard directional gate. It remains in quality scoring.
